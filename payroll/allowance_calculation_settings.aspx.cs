    ...
    public partial class allowance_calculation_settings : System.Web.UI.Page
    {
        ...
        private void loadSalaryCalculationSettings(string companyid)
        {
            ...
            sqlDB.fillDataTable("select acs.AlCalId,"+
                " case when acs.BasicAllowance=0 or acs.BasicAllowance=3 then '%' when acs.BasicAllowance=1 then 't' else 'x' end BasicAllowance ," +
                " Case when acs.MedicalAllownce=0 or acs.BasicAllowance=3 then '%' when acs.MedicalAllownce=1 then 't' else 'x' end MedicalAllownce," +
                " case when acs.FoodAllownce=0 or acs.BasicAllowance=3 then '%' when acs.FoodAllownce=1 then 't' else 'x'end FoodAllownce," +
                " Case when acs.ConvenceAllownce=0 or acs.BasicAllowance=3 then '%' when acs.ConvenceAllownce=1 then 't' else 'x' end ConvenceAllownce , " +
                " Case when acs.HouseRent=0 or acs.BasicAllowance=3 then '%' when acs.HouseRent=1 then 't' else 'x' end HouseRent, " +
                " Case when acs.TechnicalAllowance=0 or acs.BasicAllowance=3 then '%' when acs.TechnicalAllowance=1 then 't' else 'x' End TechnicalAllowance," +
                " Case when acs.OthersAllowance=0 or acs.BasicAllowance=3 then '%' when acs.OthersAllowance=1 then 't' else 'x' end OthersAllowance," +
                " Case when acs.ProvidentFund=0 or acs.BasicAllowance=3 then '%' when acs.ProvidentFund=1 then 't' else 'x' end ProvidentFund ,acs.SalaryType," +
